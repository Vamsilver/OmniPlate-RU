...
flowchart TD
    ...
    S2 --> S3["ЭТАП 3: Обучение моделей — Детектор и OCR (В РАБОТЕ)"]
...
- [x] **3.3.2. Обучение OCR на кропах синтетики и реальных номеров** (Завершено: 2026-09-15 | Артефакты: `models/ocr_lprnet_best.pt`, `models/ocr_lprnet_best.onnx` | Метрики/Статус: Val Seq Acc 90.04% [T1: 99%, 1A: 100%, 1B: 78%], Char Acc 98.6%, ONNX PASS, End-to-End Latency 9-21ms):
  - Обучен легковесный LPRNet на RTX 5080 с OneCycleLR и детерминированным сидом (90 эпох, batch 64, время эпохи ~1.4 с). Авто-экспорт в ONNX и валидация через ONNX Runtime пройдены. Калиброваны фильтры масштаба в пайплайне под близкие планы.
