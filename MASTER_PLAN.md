...
- [ ] **1.6. Настройка рабочего Python-окружения на основном ПК**:
  - Создание виртуального окружения `.venv`.
  - Установка `torch` с CUDA, `ultralytics`, `opencv-python`, `onnxruntime-gpu`.
  - Фиксация окружения в `requirements.txt`.
...
- [ ] **2.1.1. Модуль шрифта ГОСТ**:
  - Подключение/генерация векторных символов шрифта по ГОСТ Р 50577-2018 (буквы, цифры, надпись `RUS`, флаг РФ).
- [ ] **2.1.2. Рендеринг плоских шаблонов**:
  - `type1` (белый однострочный, $520 \times 112$).
  - `type1a` (белый двухстрочный квадрат, $290 \times 170$).
  - `type1b` (желтый однострочный, $520 \times 112$, без флага).
- [ ] **2.1.3. Физические текстуры и артефакты**:
  - Объемный эффект тиснения букв (embossing, свет/тень).
  - Блики, неравномерное освещение, зернистость сенсора, грязь, брызги, болты крепления.
- [ ] **2.1.4. 3D-проекция (Warp) на фоны**:
  - Случайный поворот в пространстве (yaw, pitch, roll).
  - Аналитический расчет точных координат 4 углов (`quad`) и BBox.
  - Врезка на фотографии автомобилей / парковок с альфа-блендингом.
- [ ] **2.1.5. CLI интерфейс и детерминированность**:
  - Поддержка аргументов `--count 5000 --seed 42 --output dataset/`.
  - Автоматическая генерация `dataset/images/synthetic/`, `labels/*.txt` и строк в `meta.csv`.
...
- [ ] **2.2.2. Автоматический конвейер приватности (Face Blur)**:
  - Детекция лиц на всех кандидатах.
  - Размытие (`cv2.GaussianBlur`) лиц пешеходов/водителей; отсев кадров, где человек в центре.
