...
# The absolute dense goldmine: Generations and sub-models
MODELS = [
    'toyota/mark_x', 'toyota/chaser', 'toyota/mark_ii', 'toyota/cresta',
    'toyota/crown', 'toyota/caldina', 'toyota/bb', 'toyota/probox',
    'toyota/succeed', 'toyota/wish', 'toyota/vitz', 'nissan/cube',
    'nissan/stagea', 'honda/stepwgn', 'honda/fit', 'subaru/forester'
]
...
def sprint():
    target_count = 128  # Target: 128 in new_1a_verified + 29 old = 157 total!
    ...
    # Iterate through pages 8 to 15
    for page in range(8, 16):
        if counter >= target_count:
            break
            
        print(f"\n==================== SPRINT PAGE {page} ====================", flush=True)
        for model in MODELS:
            if counter >= target_count:
                break
