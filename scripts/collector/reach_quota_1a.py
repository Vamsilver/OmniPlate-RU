...
# The 15 absolute champion models for 2-row plates in Russia
TOP_MODELS = [
    'toyota/chaser', 'toyota/mark_ii', 'toyota/cresta', 'toyota/crown',
    'nissan/cube', 'toyota/bb', 'toyota/caldina', 'toyota/succeed',
    'toyota/probox', 'toyota/wish', 'toyota/vitz', 'nissan/stagea',
    'honda/stepwgn', 'subaru/forester', 'toyota/harrier', 'toyota/corolla_fielder',
    'toyota/altezza', 'nissan/skyline', 'nissan/laurel', 'nissan/cefiro'
]
...
def run():
    target_count = 145 # Total in new_1a_verified
    ...
    print(f"[*] Current verified crops: {counter} | Unique plates: {len(plate_counts)} | Target: {target_count}", flush=True)

    print("[*] Initializing AI Pipeline (RTX 5080) and FaceBlurrer...", flush=True)
    pipeline = OmniPlatePipeline(device="cuda")
    blurrer = FaceBlurrer()
    
    # Iterate pages 2 to 7 for top models
    for page in range(2, 8):
        if counter >= target_count and len(plate_counts) >= 55:
            break
            
        print(f"\n==================== SCANNING CATALOG PAGE {page} ====================", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"[+] Reached Target! Verified Crops: {counter}, Unique Plates: {len(plate_counts)}", flush=True)
    print(f"==========================================", flush=True)
